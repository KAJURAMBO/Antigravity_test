...
class _BoardTabState extends State<BoardTab> {
        ...
        return StatefulBuilder(
          builder: (context, setModalState) {
            void safeSetState(VoidCallback fn) {
              if (isModalOpen && mounted) setModalState(fn);
            }
            ...
            void _listen() async {
              if (!isListening) {
                ...
                if (available) {
                  safeSetState(() => isListening = true);
                  speech.listen(
                    onResult: (val) => safeSetState(() {
  ...
  void _showTaskModal(BuildContext context,
      {TaskModel? task, bool isAiInput = false}) {
    final theme = context.read<ThemeProvider>().theme;
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text(isEditing ? 'Edit Mission' : 'Create New Mission',
                          style: TextStyle(
                              fontSize: 24,
                              fontWeight: FontWeight.bold,
                              color: theme.text)),
                              ...
                              Row(
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () async {
                                      if (refineController.text.trim().isEmpty)
                                        return;
                                      final feedback =
                                          refineController.text.trim();
                                      setModalState(() {
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    style: TextStyle(
                        color: theme.text, overflow: TextOverflow.ellipsis),
                    decoration: InputDecoration(
                      labelText: 'Assign To',
                      labelStyle: TextStyle(color: theme.textDim),
                      filled: true,
                    ...
                    items: [
                      ...
                      ...members
                          .where((m) =>
                              m.id != context.read<ApiService>().user?.id)
                          .map((m) => DropdownMenuItem(
                              value: m.id, child: Text(m.fullName ?? m.email))),
                    ],
                    onChanged: (val) =>
                    ...
                    child: ElevatedButton(
                      onPressed: isSaving
                          ...
                          : () async {
                              ...
                              bool success;
                              if (isEditing) {
                                success = await api.updateTask(task.id!,
                              ...
                              if (success && mounted) {
                                ...
                                if (mounted) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                        content: Text(
                                            'Failed to add mission. Please try again!'),
                                        backgroundColor: Colors.redAccent),
                                  );
                                }
                              }
                      ...
                      child: isSaving
                          ...
                          : Text(isEditing ? 'Update Task' : 'Launch Task',
  ...
  List<TaskModel> _getFilteredTasks(ApiService apiService) {
    ...
    return tasks.where((t) {
      final isAssignedToMe = t.assigneeId == user?.id ||
          (t.assigneeId == null && t.userId == user?.id);
      final isDelegatedByMe = t.userId == user?.id &&
          t.assigneeId != null &&
      ...
      appBar: AppBar(
        title: Text(
          ...
          style: TextStyle(
              ...
              color: _isSelectMode ? Colors.redAccent : theme.text),
        ),
        backgroundColor: Colors.transparent,
        elevation: 0,
        actions: [
          if (_isSelectMode) ...[
            ...
            IconButton(
              ...
              onPressed:
                  _selectedIds.isEmpty ? null : () => _bulkDelete(context),
            ),
            IconButton(
      ...
      body: Column(
        children: [
          SingleChildScrollView(
            ...
            child: Row(
              children: ['active', 'backlog', 'done', 'future', 'delegated']
                  .map((status) {
                                ...
                                if (_isSelectMode)
                                  ...
                                  Container(
                                    ...
                                    child: Icon(
                                      ...
                                      color:
                                          isDone ? Colors.green : theme.primary,
                                      size: 24,
                                    ),
                                  ),
                                const SizedBox(width: 16),
                                        ...
                                        children: [
                                          ...
                                          if (task.assigneeId != null &&
                                            ...
                                            _buildPill(
                                              icon: Icons.person,
                                              text:
                                                  "Assigned To: ${apiService.members.where((m) => m.id == task.assigneeId).firstOrNull?.fullName ?? 'Agent'}",
